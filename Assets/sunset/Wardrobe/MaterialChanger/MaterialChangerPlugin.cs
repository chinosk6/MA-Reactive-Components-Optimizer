...
[assembly: ExportsPlugin(typeof(sunset.Wardrobe.MaterialChanger.MaterialChangerPlugin))]
namespace sunset.Wardrobe.MaterialChanger
{
    public class MaterialChangerPlugin : Plugin<MaterialChangerPlugin>
    {
        public override string QualifiedName => "dev.sunset.animator-as-code.wardrobe.material-changer";
        public override string DisplayName => "Sunset Material Changer";

        private const string SystemName = "SSTMaterialChanger";
        ...
        private void Generate(BuildContext ctx)
        {
            ...
            var components = ctx.AvatarRootTransform.GetComponentsInChildren<MaterialChangerModel>(true);
            ...
            var generatingData = GetPreGenerateWithParams(components);
            ...
            {
                Debug.Log($"Generating material changer for param: {param}");
                
                var currentTree = GenerateMaterialSwitchAnimation(aac, layer, param, preGenerateItems);
                rootTree.WithAnimation(currentTree, baseWeight);
                
                // 生成完成后销毁
                foreach (var preGenerateItem in preGenerateItems)
                {
                    if (!preGenerateItem.MaMaterialSetter.Inverted)  // 反转条件不处理
                    {
                        Object.DestroyImmediate(preGenerateItem.MaMaterialSetter); 
                    }
                    Object.DestroyImmediate(preGenerateItem.MaterialChangerModel);
                }
            }
            
            layer.NewState("MaterialSetterBlend")
        ...
        private AacFlBlendTree1D GenerateMaterialSwitchAnimation(AacFlBase aac, AacFlLayer layer, string paramName, 
            List<PreGenerateMaterialSetterItem> preGenerateItems)
        {
            ...
            foreach (var preGenerateItem in preGenerateItems)
            {
                ...
                if (!preGenerateItem.MaMaterialSetter.Inverted)  // 反转条件不处理
                {
                    foreach (var materialSwitchObject in preGenerateItem.MaMaterialSetter.Objects)
                    {
                        ...
                        var obj = (GameObject)targetObjectField?.GetValue(materialSwitchObject.Object);
                        if (obj == null) continue;
                    
                        var objRenderer = obj.GetComponent<SkinnedMeshRenderer>();
                        if (objRenderer == null) continue;
                    
                        currClip = currClip.SwappingMaterial(objRenderer, materialSwitchObject.MaterialIndex,
                            materialSwitchObject.Material);
                    }
                }
        ...
        private Dictionary<string, List<PreGenerateMaterialSetterItem>> GetPreGenerateWithParams(MaterialChangerModel[] components)
        {
            var preGenerate = new Dictionary<string, List<PreGenerateMaterialSetterItem>>();
            var generatedParams = new List<string>();
            var usedParamValues = new Dictionary<string, List<float>>();
            
            foreach (var component in components)
            {
                if (!component.enableOptimization) continue;
                ...
                var maMaterialSetter = materialOptimizationObject.GetComponent<ModularAvatarMaterialSetter>();
                if (maMenuItem == null || maMaterialSetter == null) continue;
                ...
                {
                    preGenerate[paramName].Add(new PreGenerateMaterialSetterItem(component, maMenuItem, maMaterialSetter));
                }
                else
                {
                    preGenerate.Add(paramName, new List<PreGenerateMaterialSetterItem>(){ new (component, maMenuItem, maMaterialSetter) });
                }
            }
    ...
    internal class PreGenerateMaterialSetterItem
    {
        public MaterialChangerModel MaterialChangerModel { get; set; }
        public ModularAvatarMenuItem MaMenuItem { get; set; }
        public ModularAvatarMaterialSetter MaMaterialSetter { get; set; }

        public PreGenerateMaterialSetterItem(MaterialChangerModel materialChangerModel, ModularAvatarMenuItem maMenuItem,
            ModularAvatarMaterialSetter maMaterialSetter)
        {
            MaterialChangerModel = materialChangerModel;
            MaMenuItem = maMenuItem;
            MaMaterialSetter = maMaterialSetter;
        }
    }
